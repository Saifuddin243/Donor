...
<head>
    ...
    <title>Pickup at Shop Information</title>
    <style>
        body {
        ...
        .select-button {
            width: 100%;
            padding: 10px;
            background-color: #e74c3c; /* Darker red for pickup information */
            color: #fff;
            border: none;
            border-radius: 5px;
            cursor: pointer;
...
<body>
    ...
    try {
        ...
        // Fetch pickup at shop information from the database
        $query = "SELECT * FROM donorDetails WHERE pickupOption = 'pickup' ORDER BY id DESC";
        $stmt = $db->query($query);
        $receiverData = $stmt->fetchAll(PDO::FETCH_ASSOC);

        if ($receiverData) {
            echo "<h1>Pickup at Shop Information</h1>";
            ...
            echo "<h1>No Pickup Information Available</h1>";
        }
